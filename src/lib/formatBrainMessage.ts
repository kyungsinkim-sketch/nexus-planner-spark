...
export function formatBrainMessage(text: string): string {
  // Already has line breaks — skip (LLM formatted correctly)
  if (text.includes('\n')) return text;
  ...
  // 4. Clean up: remove trailing comma/period artifacts on bullet lines
  // "• 오후 3시 PT가 있습니다." → keep as-is (last item often has sentence ending)
